  ...
  return (
    <div className="w-full">
      <div className="max-w-6xl mx-auto px-8">
        ...
        <div className={`bg-white flex items-center justify-center px-0 pt-3 pb-4 transition-all duration-500 ease-out ${isScrolled ? 'fixed top-0 left-0 right-0 z-40 shadow-lg bg-white/95 backdrop-blur-sm' : 'relative'}`}>
          {/* Mobile Layout */}
          ...
          <div className={`sm:hidden absolute top-full left-0 right-0 bg-white border-t border-gray-200 transition-all duration-300 ${isMenuOpen ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'} ${isScrolled ? 'shadow-lg' : ''}`}>
            <nav className="flex flex-col py-4 px-8 space-y-4">
              ...
              <div className="relative personality-dropdown">
                ...
                <div className={`pl-4 transition-all duration-300 ${isPersonalityDropdownOpen ? 'max-h-48 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                  <Link href="/personalitytest" className={`${localGeorama.className} block py-2 text-[#A75F00] font-bold`}>
                    Take Test
                  </Link>
                  <button
                    onClick={() => {
                    ...
                    className={`${localGeorama.className} block w-full text-left py-2 text-[#4d2c00]`}
                  >
                    View Results
                  </button>
