...
const QuestionnaireCard: React.FC<QuestionnaireCardProps> = ({ onComplete, initialSet, initialQuestion }) => {
  ...
  return (
    <div className="min-h-screen bg-[#FFEDD7] flex items-center justify-center px-4 py-20" data-questionnaire-start>
      <div className="w-full max-w-4xl">
        {/* Question Indicator */}
        <p className={`${localGeorama.className} text-[#002A3C] text-base font-medium text-center mb-4`} data-question-indicator>
        ...
        {questions.map((question, index) => {
          ...
          return (
            <div key={index} className="mb-1" data-question-container={index}>
              <div className={`${containerBgColor} shadow-sm p-8 transition-colors duration-200`}>
              <p className={`${localGeorama.className} ${textColor} text-lg mb-6 text-center`}>
                {question}
              </p>
              
              {/* Radio Button Row */}
              <div className="flex items-center justify-center space-x-10">
                <span className={`${localGeorama.className} ${answers[index] ? 'text-red-400' : 'text-red-600'} text-base font-medium`}>
                  Strongly Disagree
                </span>
                  ...
                  {[1, 2, 3, 4, 5].map((value) => (
                    <label key={value} className="flex items-center">
                      <input
                        ...
                        className="w-6 h-6 border-2 border-gray-800 rounded-full appearance-none checked:bg-[#002A3C] focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    </label>
                  ))}
                </div>
                
                                  <span className={`${localGeorama.className} ${answers[index] ? 'text-green-400' : 'text-green-600'} text-base font-medium`}>
                    Strongly Agree
                  </span>
                </div>
              </div>
        ...
        <div className="text-center mt-8">
          <p className={`${localGeorama.className} text-[#002A3C] text-lg font-medium mb-4`}>
        ...
        <div className="text-center flex justify-center space-x-4">
          {currentSet > 0 && (
            <button 
              ...
              className="bg-[#002A3C] text-white px-15 py-3 font-semibold hover:bg-[#153a4f] transition-colors"
            >
              BACK
            </button>
          )}
      ...
      {showWarning && (
        <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-300">
          <div className="bg-white rounded-lg p-8 max-w-md mx-4 animate-in zoom-in-95 duration-300">
            <h3 className={`${localGeorama.className} text-[#002A3C] text-xl font-semibold mb-4 text-center`}>
              Please Complete All Questions
            </h3>
            <p className={`${localGeorama.className} text-[#002A3C] text-base mb-6 text-center`}>
              You must answer all questions in this set before proceeding to the next section.
            </p>
            <div className="text-center">
              <button
                onClick={() => setShowWarning(false)}
                className="bg-[#002A3C] text-white px-6 py-2 rounded font-semibold hover:bg-[#153a4f] transition-colors"
              >
                OK
              </button>
            </div>
